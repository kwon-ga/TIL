...
function solution(s) {
    ...
    let inputArr = s.replace(/{/g,'').replace(/}/g,'').split(',')
    const set = new Set(inputArr);
    let setData = [ ...set];

    for(let setValue of setData){
        let count =0;
        for(let inputValue of inputArr){
            if(setValue === inputValue) count++
        }
        answer[setData.length-count] = parseInt(setValue)
    }
...
function solution(s) {
    ...
    let inputArr = s.replace(/{/g,'').replace(/}/g,'').split(',').map(e=>parseInt(e)) // .sort()
    
    inputArr.reduce((accu, curr) => { 
    ...
    for(let value in sorted){
        answer.push(parseInt(sorted[value][0]))
    }
    return answer;
}
